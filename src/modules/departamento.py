...
def buscar_departamento(id_departamento):
    ...
    sql = "SELECT id_departamento, nome_departamento, orcamento_departamento FROM departamentos WHERE id_departamento = %s"
    cursor.execute(sql, (id_departamento,))
    departamento = cursor.fetchone()

    if departamento:
        id_dep, nome, orcamento = departamento
        orcamento_str = f"R$ {orcamento:,.2f}"
        print(f"\n{'ID':<5} | {'DEPARTAMENTO':<30} | {'ORÇAMENTO':<15}")
        print("-" * 55)
        print(f"{id_dep:<5} | {nome:<30} | {orcamento_str:<15}")
    else:
        print(f"\nNenhum departamento encontrado com ID {id_departamento}.")
