...
def buscar_objetivo(id_objetivo):
    conexao = conectar()
    cursor = conexao.cursor()
    sql = "SELECT id_objetivo, descricao FROM Objetivo_Viagem WHERE id_objetivo = %s"
    cursor.execute(sql, (id_objetivo,))
    resultado = cursor.fetchone()

    if resultado:
        id_obj, descricao = resultado
        print(f"\nID: {id_obj}\nDescrição: {descricao}")
    else:
        print(f"\nNenhum objetivo encontrado com ID {id_objetivo}.")
