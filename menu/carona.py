...
def buscar_carona(id_carona):
    conexao = conectar()
    cursor = conexao.cursor()

    sql = "SELECT id_carona, id_viagem, id_colaborador FROM Carona WHERE id_carona = %s"
    cursor.execute(sql, (id_carona,))
    ...
    if carona:
        id_carona, id_viagem, id_colaborador = carona
        print(f"\nCarona encontrada:")
        print(f"ID: {id_carona}")
        print(f"Viagem: {id_viagem}")
        print(f"Colaborador: {id_colaborador}")
    else:
        print(f"\n❌ Nenhuma carona encontrada com ID {id_carona}.")
