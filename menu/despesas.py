...
def buscar_despesa(id_despesas):
    conexao = conectar()
    cursor = conexao.cursor()

    sql = "SELECT id_despesas, valor, data_hora, categoria, forma_pagamento, id_viagem FROM Despesas WHERE id_despesas = %s"
    cursor.execute(sql, (id_despesas,))
    ...
    if despesa:
        ...
        print(f"❌ Nenhuma despesa encontrada com ID {id_despesas}")
