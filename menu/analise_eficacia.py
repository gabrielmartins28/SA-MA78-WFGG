...
def buscar_analise_eficacia(id_analise):
    conexao = conectar()
    cursor = conexao.cursor()
    sql = "SELECT id_analise, data_analise, id_viagem, id_colaborador, indicador_sucesso, observacao_colaborador FROM analise_eficacia_viagem WHERE id_analise = %s"
    cursor.execute(sql, (id_analise,))
    ...
    if analise:
        ...
        print(f"Análise ID {id_analise} não encontrada.")
